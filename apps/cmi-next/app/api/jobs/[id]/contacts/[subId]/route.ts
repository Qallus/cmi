...
import { updateJobContact, removeJobContact, JobError } from "@/lib/jobs/data";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string; subId: string }> }) {
  try {
    ...
    return NextResponse.json(await updateJobContact(subId, await request.json()));
  } catch (err) {

...
import { updateJobInternalUser, removeJobInternalUser, JobError } from "@/lib/jobs/data";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string; subId: string }> }) {
  try {
    ...
    return NextResponse.json(await updateJobInternalUser(subId, await request.json()));
  } catch (err) {
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string; subId: string }> }) {
  try {
    const { staff } = await requireAdmin(request);
    if (!WRITE_ROLES.includes(staff.role_slug)) return NextResponse.json({ error: "Forbidden." }, { status: 403 });
    const { subId } = await params;
    await removeJobInternalUser(subId);

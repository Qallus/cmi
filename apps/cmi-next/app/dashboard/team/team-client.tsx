...
import { Mail, Phone, Plus, Trash2, User, X } from "lucide-react";
...
export function TeamClient({ initialMembers }: { initialMembers: TeamMember[] }) {
  ...
  function openEdit(m: TeamMember) {
    setDraft({ name: m.name, slug: m.slug ?? "", role: m.role ?? "", department: m.department ?? "", bio: m.bio ?? "", tagline: m.tagline ?? "", email: m.email ?? "", phone: m.phone ?? "", profile_photo: m.profile_photo ?? "", secondary_photo: m.secondary_photo ?? "", attributes: m.attributes ?? [], availability: m.availability ?? "", sort_order: m.sort_order, status: m.status });
    ...
    <div className="p-4 md:p-6">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">{active.length} active members</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {active.map((m) => (
          <div
            key={m.id}
            className="group relative cursor-pointer overflow-hidden rounded-xl border border-border bg-card transition hover:border-accent/40 hover:shadow-md"
          ...
          >
            ...
            <div className="relative h-52 overflow-hidden bg-muted">
              {m.profile_photo ? (
                <img
                  src={hoveredId === m.id && m.secondary_photo ? m.secondary_photo : m.profile_photo}
                  alt={m.name}
              ...
              <div className="absolute bottom-3 left-3 right-3">
                ...
                {m.role && <div className="text-xs text-white/80">{m.role}</div>}
              </div>
            </div>
            <div className="p-3">
              {m.tagline && <p className="mb-2 text-xs text-muted-foreground line-clamp-2">{m.tagline}</p>}
      ...
      {inactive.length > 0 && (
        <div className="mt-8">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {inactive.map((m) => (
              <div key={m.id} className="flex cursor-pointer items-center gap-3 rounded-lg border border-border p-3 opacity-60 transition hover:opacity-100" onClick={() => openEdit(m)}>
                {m.profile_photo ? <img src={m.profile_photo} alt={m.name} className="h-9 w-9 rounded-full object-cover" /> : <div className="flex h-9 w-9 items-center justify-center rounded-full bg-muted"><User className="h-4 w-4 text-muted-foreground" /></div>}
            ...
            <div className="space-y-5 p-5">
              ...
              {(draft.profile_photo || draft.secondary_photo) && (
                <div className="flex gap-3">
                  {draft.profile_photo && <img src={draft.profile_photo} alt="Profile" className="h-20 w-20 rounded-lg object-cover" />}
                  {draft.secondary_photo && <img src={draft.secondary_photo} alt="Secondary" className="h-20 w-20 rounded-lg object-cover opacity-80" />}
                </div>
              )}
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-sm rounded-xl border border-border bg-card p-6 shadow-xl">
            ...
            <p className="mt-2 text-sm text-muted-foreground">This will permanently delete the team member.</p>

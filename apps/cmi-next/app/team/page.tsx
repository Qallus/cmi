    ...
    <article className="group relative overflow-hidden rounded-2xl border border-border bg-card transition duration-300 hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="relative z-20 p-6">
        ...
        <div className="mt-5 flex items-center gap-3 border-t border-border pt-4">
          {member.email ? (
            <a href={`mailto:${member.email}`} aria-label={`Email ${member.name}`} className="relative z-30 flex h-8 w-8 items-center justify-center rounded-lg text-muted-foreground transition hover:bg-muted hover:text-accent">
              <Mail className="h-4 w-4" />
            </a>
          ) : null}
          {member.phone ? (
            <a href={`tel:${member.phone.replace(/[^+\d]/g, "")}`} aria-label={`Call ${member.name}`} className="relative z-30 flex h-8 w-8 items-center justify-center rounded-lg text-muted-foreground transition hover:bg-muted hover:text-accent">
...
export default async function TeamPage() {
  ...
  const leadership = members.filter(member => (member.department || "").toLowerCase().includes("leadership") || (member.role || "").toLowerCase().includes("principal"));
  const staff = members.filter(member => !leadership.some(leader => leader.id === member.id));
      ...
      <main>
        ...
        <section className="bg-background py-20 lg:py-28">
          <div className="mx-auto max-w-7xl px-5 lg:px-8">
            ...
            <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
              {(leadership.length ? leadership : members.slice(0, 3)).map(member => (
        ...
        <section className="border-t border-border bg-card/40 py-20 lg:py-28">
          <div className="mx-auto max-w-7xl px-5 lg:px-8">
            ...
            <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
              {(staff.length ? staff : members.slice(3)).map(member => (

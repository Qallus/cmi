...
import { ArrowLeft, Loader2, Mic, Pencil, Send, Sparkles, Trash2 } from "lucide-react";
...
export function BriefDetail({ canvasId }: { canvasId: string }) {
  ...
  async function del() {
    ...
    setBusy(true);
    try { await api.apiDeleteCanvas(canvasId); router.push("/dashboard/canvas-briefs"); } catch (e) { setError(e instanceof Error ? e.message : "Delete failed."); setBusy(false); }
  }
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
        ...
        <div className="min-w-0 space-y-6">
          <div className="flex flex-wrap items-end justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              {isSuper && (
                <>
                  <Link href={`/dashboard/canvas/${canvasId}`} title="Edit in canvas" className="grid h-8 w-8 place-items-center rounded-lg border border-border text-muted-foreground hover:border-accent/50 hover:text-foreground"><Pencil className="h-3.5 w-3.5" /></Link>
                  <button type="button" onClick={del} disabled={busy} title="Delete" className="grid h-8 w-8 place-items-center rounded-lg border border-border text-muted-foreground hover:border-destructive/50 hover:text-destructive"><Trash2 className="h-3.5 w-3.5" /></button>
                </>
              )}
            </div>
          </div>
        ...
        <aside className="lg:sticky lg:top-6 lg:self-start">
          <div className="rounded-xl border border-border bg-card">
            <div className="border-b border-border px-4 py-3 text-sm font-semibold uppercase tracking-[0.1em] text-muted-foreground">Team Comments</div>
            <div className="max-h-[52vh] space-y-2 overflow-y-auto p-3">
              {comments.length === 0 && <p className="px-1 py-2 text-sm text-muted-foreground">No comments yet.</p>}
              {comments.map((c) => (
                <div key={c.id} className="rounded-lg border border-border px-3 py-2">
                  <div className="flex items-center justify-between gap-2 text-xs text-muted-foreground"><span className="font-medium text-foreground">{c.author_name ?? "Staff"}</span><span>{new Date(c.created_at).toLocaleDateString()}</span></div>
                  <p className="mt-1 text-sm">{c.body}</p>
                </div>
              ))}
            </div>
            <div className="border-t border-border p-3">
              <textarea value={comment} onChange={(e) => setComment(e.target.value)} rows={3} placeholder="Add a comment for the team…" className="w-full resize-none rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:border-accent" />
              <div className="mt-2 flex justify-end"><Button variant="accent" onClick={submitComment} disabled={busy || !comment.trim()}><Send className="h-4 w-4" /> Post</Button></div>
            </div>
          </div>
        </aside>
      </div>
